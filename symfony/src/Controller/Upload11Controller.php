...
class Upload11Controller extends AbstractController
{
    ...
    #[Route('/api/upload11', name: 'upload11', methods: ['GET', 'POST'])]
...
$sql = $entityManager->getRepository('App\Entity\Home')->findBy($tab);
...
if ((int)count($sql)===(int)0){
    ...
    $this->imageSize->setValue('App\Entity\Home','sekcje',(int)$Home->getId(),(string)$arraystring,(array)$newImage);
    //$this->imageSize->setSqlId((int)$catt1->getId());
    $this->imageSize->go();
    
    file_put_contents($this->containerbag->get('kernel.project_dir').'/public/bufor/homedata/'.(string)$Home->getId(),'');
    file_put_contents($this->containerbag->get('kernel.project_dir').'/public/bufor/homesekcje/'.(string)$Home->getId(),json_encode($request['postData']));
}
//wstawianie danych
else{
...
//else if (strlen($data)>0 && (int)count($sql)!==(int)0){
    ...
    //dodawanie/usuwanie zdjęć
    $this->imageSize->setValue('App\Entity\Home','sekcje',(int)$sql[0]->getId(),(string)$arraystring,(array)$newImage);
    //$this->imageSize->setSqlId((int)$catt1->getId());
    $this->imageSize->go();
    
    file_put_contents($this->containerbag->get('kernel.project_dir').'/public/bufor/homesekcje/'.(string)$sql[0]->getId(),json_encode($request['postData']));
