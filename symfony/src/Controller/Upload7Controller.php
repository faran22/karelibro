...
class Upload7Controller extends AbstractController
{
    ...
    #[Route('/api/upload7', name: 'upload7', methods: ['GET', 'POST'])]
...
if ((int)count($sql)===(int)0){
    ...
    $this->imageSize->setValue('App\Entity\Home','data',(int)$Home->getId(),(string)$data,(array)$newImage);
    //$this->imageSize->setSqlId((int)$Home->getId());
    $this->imageSize->go();
    
    file_put_contents($this->containerbag->get('kernel.project_dir').'/public/bufor/homedata/'.(string)$Home->getId(),$data);
}
//wstawianie danych
else{
...
//else if (strlen($data)>0 && (int)count($sql)!==(int)0){
    ...
    //dodawanie/usuwanie zdjęć
    $this->imageSize->setValue('App\Entity\Home','data',(int)$sql[0]->getId(),(string)$data,(array)$newImage);
    //$this->imageSize->setSqlId((int)$sql[0]->getId());
    $this->imageSize->go();
    
    file_put_contents($this->containerbag->get('kernel.project_dir').'/public/bufor/homedata/'.(string)$sql[0]->getId(),$data);
}
