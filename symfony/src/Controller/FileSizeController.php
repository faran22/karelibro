...
class FileSizeController extends AbstractController
{
    ...
    public function setValue(string $entity, string $sqlRekord, int|null $sqlId, array $owner):void
    {
        ...
        $this->sqlId=$sqlId;
        $this->aktualne=$this->toArrayImage($owner);
    ...
    $poprzednie=(array)json_decode(@file_get_contents($this->getParameter('kernel.project_dir').'/public/bufor/file1data/'.(string)$this->sqlId),true);
    
    //wyodrebnianie nazw plikow
    $this->poprzednie=$this->toArrayImage($poprzednie);
    
    
    }
    
    //wyodrebnianie nazw plikow
    public function toArrayImage(array $tt):array
    {
    
    $tab=array();
    foreach ($tt as $value){array_push($tab, $value['id']);}
    return $tab;
    ...
    protected function OwnerDirectory():void
    {
        ...
        if (!file_exists($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId())){
    
            mkdir($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId());
        ...
        if (!file_exists($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId().'/size')){
            ...
            file_put_contents($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId().'/size',json_encode(array()));
    ...
    protected function addFile($file):void
    {
        ...
        //echo 'add>'.$file;
        if (!file_exists($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId().'/'.$file)){
        
        $data=array();
        $data['size']=(int)@filesize($this->getParameter('kernel.project_dir').'/public/picture/'.$file);
        
        file_put_contents($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId().'/'.$file,json_encode($data));
        touch($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId().'/'.$file);
    ...
    protected function removeFile($file):void
    {
        ...
        touch($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId().'/'.$file,0);
        ...
        if (flock($fp, LOCK_EX)){//sleep(15);
            ...
            if ($typ===true){
                ...
                if (!isset($data[$file])){$new=true;
                    $t=(array)json_decode(@file_get_contents($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId().'/'.$file),true);
                    $data[$file]=$t['size'];
                }
            ...
            if ($new===true){//echo 'zapis';
                file_put_contents($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId().'/size',json_encode($data));
                touch($this->getParameter('kernel.project_dir').'/public/bufor/buforfile1/'.(string)$this->apiauthorizecontroller->getUser()->getId().'/size');
            }
